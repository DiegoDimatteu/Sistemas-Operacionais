...
void *imprimir_matriz(void *arg){
  ...
  while(1){
    ...
    for(i = 0; i < 30; i++){//Identa as linhas para os numeros menores que 10 ficarem com dois algarismos
      if(i == 0){
        printf("   0%d ", i);
      }
      else{
    ...
    printf("\n");
    for(i = 0; i < MAX; i++){
      for(j = 0; j < MAX; j++){//identa as colunas para os numeros menores que 10 ficarem com dois algarismos
        if(j == 0){
          if (i <= 9){
            printf("0%d ",i);
          }
    ...
    printf ("%s\n", buff);//Imprime o horário embaixo do mapa
...
void *incendio(void *arg)//Função que cria os incêndios
{
  ...
  while(1)
  {
    ...
    int j = rand() % MAX;
    if ((*floresta)[i][j] == 'T'){//Se for uma posição de thread ela mata a thread
...
void *identificar()
{
      ...
      for(i = (dado[t].x) - 1; i <= (dado[t].x) + 1; i++){
        for(j = (dado[t].y) - 1; j <= (dado[t].y) + 1; j++){
          if(floresta[i][j] == '@' && dado[t].condicional == 0 && dado[t].morta == 0){//Se ela não tiver dado armazedado internamente, não estiver morta e tiver achado um icendio
            ...
            num = (10 * ((((dado[t].x) + 2)/3) - 1)) + (((dado[t].y) + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
            dado[t].id = num;
            dado[t].incendio_x = i;
            dado[t].incendio_y = j;
  ...
  sleep(1);//Espera 1 segundo para fazer a verificação conforme o trabalho pede
  }
}
...
void passagem(int atual, int a, int b, int c, int d){//Passagem por forma recursiva
  ...
  if((dado[a].condicional == 0) && (dado[a].morta == 0) && (dado[a].envio_x != dado[atual].x) && (dado[a].envio_y != dado[atual].y) && floresta[dado[atual].incendio_x][dado[atual].incendio_y] == '#'){
    num = (10 * ((((dado[a].x) + 2)/3) - 1)) + (((dado[a].y) + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
    ...
    if(dado[a].x != 1 && dado[a].x != 28 && dado[a].y != 1 && dado[a].y != 28){//Se não chegar na borda ele continua fazendo a recursividade
  ...
  if(dado[b].condicional == 0 && dado[b].morta == 0 && dado[b].envio_x != dado[atual].x && dado[b].envio_y != dado[atual].y && floresta[dado[atual].incendio_x][dado[atual].incendio_y] == '#'){
    num = (10 * ((((dado[b].x) + 2)/3) - 1)) + (((dado[b].y) + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
    ...
    if(dado[b].x != 1 && dado[b].x != 28 && dado[b].y != 1 && dado[b].y != 28){//Se não chegar na borda ele continua fazendo a recursividade
  ...
  if(dado[c].condicional == 0 && dado[c].morta == 0 && dado[c].envio_x != dado[atual].x && dado[c].envio_y != dado[atual].y && floresta[dado[atual].incendio_x][dado[atual].incendio_y] == '#'){
    num = (10 * ((((dado[c].x) + 2)/3) - 1)) + (((dado[c].y) + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
    ...
    if(dado[c].x != 1 && dado[c].x != 28 && dado[c].y != 1 && dado[c].y != 28){//Se não chegar na borda ele continua fazendo a recursividade
  ...
  if(dado[d].condicional == 0 && dado[d].morta == 0 && dado[d].envio_x != dado[atual].x && dado[d].envio_y != dado[atual].y && floresta[dado[atual].incendio_x][dado[atual].incendio_y] == '#'){
    num = (10 * ((((dado[d].x) + 2)/3) - 1)) + (((dado[d].y) + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
    ...
    if(dado[d].x != 1 && dado[d].x != 28 && dado[d].y != 1 && dado[d].y != 28){//Se não chegar na borda ele continua fazendo a recursividade
...
void *thread_central()//Faz a verificação constante para que qualquer thread da borda que identificou o incendio possa chamar o bombeiro
{
  int i, j, num;
  while(1)
  { 
    for(i = 1; i < 29; i += 27){//Verifica a borda parando na linha e olhando as colunas
      for(j = 1; j < 29; j += 3){
        num = (10 * (((i + 2)/3) - 1)) + ((j + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
        if(floresta[dado[num - 1].incendio_x][dado[num - 1].incendio_y] == '#' && dado[num - 1].morta == 0 && dado[num - 1].condicional == 1){
          ...
          fclose(fptr);//Escreve a mensagem antes de chamar o bombeiro
    ...
    for(j = 1; j < 29; j += 27){//Ele verifica a borda fazendo o contrário, para na coluna e olha as linhas
      for(i = 4; i < 28; i += 3){
        num = (10 * (((i + 2)/3) - 1)) + ((j + 2)/3);//Função criada para pegar o número da thread a partir de sua posição
        if(floresta[dado[num - 1].incendio_x][dado[num - 1].incendio_y] == '#' && dado[num - 1].morta == 0 && dado[num - 1].condicional == 1){
...
void bombeiro(int x, int y)//Apaga os incêndios
{
  fptr = fopen("incendios.log.txt", "a"); 
  fprintf(fptr, "Bombeiro apagou o incendio na posicao [%d][%d]\n", x, y);//Escreve no txt quando apaga o incêndio
  fclose(fptr);
  floresta[x][y] = '-' ;
}